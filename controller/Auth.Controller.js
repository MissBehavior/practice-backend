...
module.exports = {
  register: async (req, res, next) => {
    ...
    try {
      ...
      const accessToken = await signAccessToken(savedUser.id, savedUser.isAdmin);
      const refreshToken = await signRefreshToken(savedUser.id, savedUser.isAdmin);
  ...
  login: async (req, res, next) => {
    try {
      ...
      const refreshToken = await signRefreshToken(user.id, user.isAdmin);
  ...
  refreshToken: async (req, res, next) => {
    ...
    try {
      ...
      const accessToken = await signAccessToken(userId, userIsAdmin);
      const refToken = await signRefreshToken(userId, userIsAdmin);
